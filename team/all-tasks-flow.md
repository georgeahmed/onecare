...
Complete Task Catalog (All Departments)
- This catalog lists every task by ID with its title for quick lookup.
- Source of truth remains the individual task cards under `team/*/tasks/`.

Backend — Orchestrator (BE-01)
- BE-01.1 — Add zero-trust gate (verify/auth/consent) plumbing
- BE-01.2 — Load typed config + enforce floors/ceilings
- BE-01.3 — Publish triage.input envelope on SAFE
- BE-01.4 — Emit audit event on success/deny
...
- BE-01.9 — Orchestrator state machine skeleton
- BE-01.10 — HTTP ingress validation + body size limits
- BE-01.11 — Observability: correlationId propagation, metrics, and spans
- BE-01.12 — Security hardening (SSRF, sanitation, consent)
- BE-01.13 — Audit ledger resiliency (async WORM, redaction)
- BE-01.14 — Backpressure + time budgets
- BE-01.15 — Contract compliance & codegen (schemas → types → validators → CI gate)
- BE-01.16 — DLQ and retry policy (bounded retries, poison queue, idempotent handlers)
- BE-01.17 — Rate limiting & abuse protection
- BE-01.18 — Health, readiness, liveness, and graceful shutdown
- BE-01.19 — Fault injection tests (timeouts, partial failures, slow responses)
- BE-01.20 — Privacy/PII/PHI minimization and redaction
- BE-01.21 — Performance baselines and budgets (p50/p95 latency, throughput)
- BE-01.22 — Documentation & ADRs (orchestrator, resilience, error taxonomy)
- BE-01.1H — Zero‑trust gate hardening (redaction, rate limit, correlation)
- BE-01.2H — Orchestrator config schema + startup validation
- BE-01.3H — Reliable publish (ack/retry) and DLQ prep
- BE-01.4H — Audit WORM semantics + redaction policy
...
- BE-02.5 — Connection resilience and backoff
- BE-02.5a — Connection resilience (reconnect jitter, backoff, status events)
- BE-02.6 — Delivery semantics & idempotency
- BE-02.6a — At-least-once semantics + dedupe keys
- BE-02.7 — Ordering & partitioning
- BE-02.8 — Flow control & backpressure
- BE-02.9 — Security hardening for bus
- BE-02.10 — Bus observability (latency, errors, spans; correlation)
- BE-02.11 — Health/readiness integration for bus
- BE-02.12 — Contract enforcement for envelopes/topics
- BE-02.13 — Advanced DLQ/retry policy
- BE-02.14 — Adapter parity tests vs MemoryBus
- BE-02.15 — Documentation & ADRs for bus
- BE-02.16 — Multi-tenant isolation & quotas (Bus)
- BE-02.17 — Message size & compression policies (Bus)
- BE-02.18 — Partitioning strategy doc + tests (Bus)
- BE-02.19 — Performance baselines & soak (Bus)
- BE-02.20 — Runbooks & alerting (Bus)

Backend — Triage (BE-03)
- BE-03.1 — Scoring function skeleton with config weights
...
- BE-03.5 — Queue notifier adapter
- BE-03.6 — Contract-first triage models and validators
- BE-03.7 — Event-driven ingress (triage.input consumer)
- BE-03.8 — SLA aging and escalation
- BE-03.9 — Fallbacks for missing ML (rules-based scoring)
- BE-03.10 — Observability (correlation, metrics, spans)
- BE-03.11 — Security & privacy (PHI minimization)
- BE-03.12 — Performance baselines (latency, throughput)
- BE-03.13 — DLQ and poison-message quarantine
- BE-03.14 — Health/readiness for triage service
- BE-03.15 — Documentation & ADRs (scoring, dedup, assignment)

Backend — Booking (BE-04)
- BE-04.1 — GP Connect client interface + env wiring
- BE-04.2 — booking.search handler skeleton
- BE-04.3 — Appointment create + conflict handling
- BE-04.4 — Contract-first: booking search/request/response and appointment-created schemas; codegen + validators + contract tests
- BE-04.5 — Idempotent booking & dedupe (natural key; exactly-once publish)
- BE-04.6 — DLQ and retry policy for booking
- BE-04.7 — Outbound guardrails (GP Connect/FHIR)
- BE-04.8 — Observability (search/booking latency; spans; correlation)
- BE-04.9 — Backpressure & rate limits (booking)
- BE-04.10 — Health/readiness & graceful shutdown (booking)
- BE-04.11 — Privacy/PII minimization (booking)
- BE-04.12 — Performance baselines (booking)
- BE-04.13 — Fault injection tests (booking)
- BE-04.14 — Documentation & ADRs (booking)
- BE-04.15 — Documentation & ADRs (booking)

Backend — Pharmacy Router (BE-05)
- BE-05.1 — Eligibility rules config + evaluator
- BE-05.2 — CPCS adapter interface
- BE-05.3 — Referral path + outcome write‑back
- BE-05.4 — Contract-first: referral/outcome schemas, validators, codegen
- BE-05.5 — Idempotent referral and outcome processing
- BE-05.6 — Outbound guardrails to CPCS
- BE-05.7 — Patient notification adapter
- BE-05.8 — DLQ and retry policy
- BE-05.9 — Observability for pharmacy router
- BE-05.10 — Health/readiness + graceful shutdown (pharmacy)
- BE-05.11 — Documentation & ADRs (pharmacy router)
- BE-05.12 — GP fallback path
- BE-05.13 — Performance baselines (pharmacy)
- BE-05.14 — Fault injection tests (pharmacy)
- BE-05.15 — Rate limits & backpressure (pharmacy)

Backend — Access & Capacity (BE-06)
- BE-06.1 — Portal Uptime Guard schedule
- BE-06.2 — OOH deferral queue (persisted with TTL; flush-on-core-hours)
- BE-06.3 — ShapeCapacity micro-release (bounded, audited)
- BE-06.4 — Portal notify event (contract-first, idempotent, DLQ)
- BE-06.5 — Capacity telemetry adapter (pluggable, health-aware)
- BE-06.6 — Scheduler resilience & idempotency (singleton, jitter, skew tolerance)
- BE-06.7 — Observability for access/capacity (logs, metrics, spans)
- BE-06.8 — Front-door rate limiting (tenant/account) for access portal
- BE-06.9 — Portal event DLQ & retries (bounded, idempotent)
- BE-06.10 — Health/readiness/liveness + graceful shutdown (schedulers)
- BE-06.11 — Fault injection tests (ticks, failures, clock drift)
- BE-06.12 — Privacy/PII/PHI minimization (access-gate)
- BE-06.13 — Performance baselines (tick latency, flush throughput)

Backend — ICS & Automation (BE-07)
- BE-07.1 — ICS ingress validation + routing policy skeleton
- BE-07.2 — Workflow Automation triggers (policy-driven)
...
- BE-07.13 — Contract validators & codegen (ICS & Automation)
- BE-07.14 — Rate limiting & abuse protection (ICS Hub)
- BE-07.15 — Performance baselines (ICS & Automation)
- BE-07.16 — Documentation & ADRs (ICS Hub & Automation)
- BE-07.17 — Health/readiness + graceful shutdown (ICS)
- BE-07.18 — Fault injection tests (ICS hub)

Integrations (IN-01/IN-02/IN-03)
- IN-01.1 — FHIR client skeleton + env wiring (baseUrl, auth)
- IN-01.2 — Bundle upsert (transaction) with retry/timeout
- IN-01.3 — Create methods: Task, Appointment, DocumentReference
...
- IN-01.10 — Idempotent FHIR operations (ETag/If-Match, 409/412 handling)
- IN-01.11 — FHIR search/pagination helpers (429 handling)
- IN-01.12 — Observability (correlationId, metrics, spans; PHI-safe logs)
- IN-01.13 — Health/readiness probes (FHIR/Object Store/OIDC)
- IN-01.14 — Contract & content negotiation (FHIR JSON; Bundle.transaction)
...
- IN-03.5 — Observability for CPCS/ICS (structured logs, latency histograms, error counters, spans)
- IN-03.6 — ICS org→endpoint map and routing policy (allowlist; per-org limits)
- IN-03.7 — Security & SSRF guardrails (CPCS/ICS)
- IN-03.8 — Contract-first ICS mapping (schemas/codegen; compiled validators; contract tests)
- IN-03.9 — Idempotency & dedupe (external calls and acks)
- IN-03.10 — DLQ and retry policy for ICS events
- IN-03.11 — Health/readiness and graceful shutdown (Integrations)
- IN-03.12 — Fault injection tests (timeouts, partial failures, CB open, retries → DLQ)
- IN-03.13 — Performance baselines (p50/p95; soak)
- IN-03.14 — Privacy/PII minimization (Integrations)
- IN-03.15 — Credential management & rotation readiness
- IN-03.16 — Documentation & ADRs (CPCS/ICS integration)
...
- SRE-01.8 — Supply chain: SBOM + vulnerability scanning in CI
- SRE-01.9 — Data retention/minimization: log/event retention policies + purge tooling
- SRE-01.10 — Observability stack: OTEL, Prometheus/Grafana, logs (correlationId)
- SRE-01.11 — SLOs & alerting (latency p95, error rates, DLQ growth, readiness flaps)
- SRE-01.12 — Secrets manager integration (Vault/KMS), sealed‑secrets, rotation runbook
- SRE-01.13 — Network security (cert‑manager, mTLS, Ingress, egress allowlist, NetworkPolicies)
...
- SRE-02.7 — Trace sampling policy (tail-based; error/latency bias)
...
- SEC-01.1 — Logging redaction utility
- SEC-01.2 — AuthZ matrix documentation (scopes/actions)
- SEC-01.3 — CSP baseline and security headers policy
- SEC-01.4 — Dependency scanning gates and license policy
- SEC-01.5 — Secrets scanning in CI and pre-commit
- SEC-01.6 — Egress allowlist enforcement and SSRF guard
- SEC-01.7 — Pen-test checklist and hardening backlog
- SEC-02.1 — Threat model & data flow (STRIDE)
- SEC-02.2 — Data classification & retention policy
- SEC-02.3 — DPIA template & privacy impact checks
- SEC-02.4 — mTLS & cert policy (pinning/rotation)
- SEC-02.5 — OIDC/JWT verification hardening
- SEC-02.6 — Secrets policy & rotation SLAs
- SEC-02.7 — SAST/DAST gates (Semgrep/CodeQL; OWASP ZAP)
- SEC-02.8 — Vulnerability management (CVSS thresholds; triage/patch SLAs)
- SEC-02.9 — Access reviews & RBAC least-privilege
- SEC-02.10 — Incident Response plan & drills (IRP)
- SEC-02.11 — Audit logging policy (PHI‑safe; retention; WORM)
- SEC-02.12 — Supply chain policy (SBOM/signing; image policy admission)
- SEC-02.13 — Egress/SSRF policy
- SEC-02.14 — Secure coding guidelines & checklists (Node/Python)
- SEC-02.15 — Vendor risk + third‑party assessment
- SEC-02.16 — Security & privacy training
- SEC-02.17 — Compliance mapping (DSPT; DCB 0129/0160) & evidence
- SEC-03.1 — Secure SDLC policy & PR checklist integration
- SEC-03.2 — Code scanning (Semgrep/CodeQL) curated rules + severity gates
- SEC-03.3 — Dependency update automation with security gates
- SEC-03.4 — Secrets prevention hooks (pre‑commit/pre‑receive)
- SEC-03.5 — HTTP/API fuzzing harness (property‑based + ZAP baseline)
- SEC-03.6 — SSRF test suite (egress mocks; private IP redirection tests)
- SEC-03.7 — AuthZ matrix → automated tests
- SEC-03.8 — WAF/rate‑limit/DoS policies for public endpoints
...
- MO-01.6 — Base dashboards for latency (p50/p95), error rate, throughput
- MO-01.7 — Vulnerability & secrets scanning in CI
- MO-01.8 — Rollout strategies: blue/green, canary, and shadow
- MO-01.9 — Autoscaling & concurrency (HPA/Knative)
- MO-01.10 — GPU/CPU scheduling & resource classes
- MO-01.11 — Model/version registry integration
- MO-01.12 — Shadow evaluation harness
- MO-01.13 — SLOs & alerting (p95 latency, error rate, throttle, OOM)
- MO-01.14 — Container hardening (non-root, distroless, read-only FS, seccomp, capabilities)
- MO-01.15 — SBOM + provenance (SLSA/cosign)
- MO-01.16 — Secrets management integration (Vault/KMS) & rotation
- MO-01.17 — Drift/quality monitoring hooks (handoff)
- MO-01.18 — Chaos/resiliency drills (kill pods, network faults)
- MO-01.19 — Cost/performance profiling (CPU/GPU utilization, cold start, memory)
- MO-01.20 — Documentation & runbooks (deploy, rollback, DR)
- MO-02.1 — Feature store backend selection (ADR)
- MO-02.2 — Implement FeatureStore.put/get in TS (@onecare/ports impl)
- MO-02.3 — Telemetry hook to log features from triage/safety into store
- MO-02.4 — Drift metrics (PSI/KL or mean/std diff) and alerts
- MO-02.5 — Retention policy + purge job script
- MO-02.6 — Unit tests and sample report generation
- MO-02.7 — Reliable ingestion (at-least-once, idempotency, retries+jitter, DLQ)
- MO-02.8 — Freshness SLI/SLO (max age per feature) and alerting
- MO-02.9 — Dashboards for feature health and runbooks
- MO-02.10 — Online store health/readiness + cache strategy
- MO-02.11 — PIT query helper for training sets (leakage‑safe) + examples
- MO-02.12 — Training/serving skew monitors + alerting thresholds
- MO-02.13 — Privacy & governance checks (features)
- MO-02.14 — Performance/load tests (online get/put; cost/perf)
- MO-02.15 — SDK/notebook examples for ML users
- MO-02.16 — Lifecycle alignment (retention/compaction) with Data Eng
...
- DE-01.18 — Performance/load testing (throughput, backpressure, cost/perf)
- DE-01.19 — Documentation & ADRs (storage, partitioning, DQ policy, reprocessing)
...
- FE-01.17 — Frontend observability (correlationId, structured logs, RUM)
...
- FE-02.8 — Performance: virtualization and prefetch
...
- FE-03.4 — Interpreter preferences UI section (config-driven)
- FE-03.5 — Accessibility audit tooling in CI
- FE-03.6 — RTL & bidirectional text support
- FE-03.7 — ICU messages + extraction pipeline
- FE-03.8 — Pseudo‑localization + missing‑key detection
- FE-03.9 — Locale date/number/timezone formatting
...
- FE-03.12 — High‑contrast theme tokens and contrast audits
...
- FE-03.15 — Localized error/envelope mapping
- FE-03.16 — Performance: lazy‑load locale bundles; code‑split translations
- FE-03.17 — Interpreter preferences: UI/validation/persistence
- FE-03.18 — Fallback logic & offline locale cache
- FE-03.19 — A11y/i18n QA matrix & test plan
- FE-03.20 — Localization authoring guide (glossary, style, review)

QA Automation (QA-01/QA-02)
- QA-01.1 — Schema validation harness for TS events (Ajv)
- QA-01.2 — Contract tests for triage.input, tasks.created, appointment.created
- QA-01.3 — E2E test: portal submission → orchestrator → triage Task
- QA-01.4 — E2E test: booking search/create write-back
- QA-01.5 — OpenAPI validation for Python endpoints (expand CI script)
- QA-01.6 — Contract tests for pharmacy and ICS
- QA-01.7 — Property-based and fuzz tests for contracts
- QA-01.8 — E2E fault injection (timeouts, CB open, partial failures)
- QA-01.9 — DLQ path verification
- QA-01.10 — Backpressure and rate limit tests
- QA-01.11 — Privacy/redaction tests (logs/events PHI-free)
- QA-01.12 — CorrelationId propagation audit
- QA-01.13 — Telephony parity E2E
- QA-01.14 — Synthetic load smoke (k6/newman)
- QA-01.15 — Hermetic test environment
- QA-01.16 — Flaky test detection and quarantine
- QA-01.17 — Test data management (fixtures, anonymization, rotation)
- QA-01.18 — Contract drift monitor (schema/codegen diff gates; snapshots)
- QA-01.19 — Accessibility checks in E2E (axe)
- QA-01.20 — QA runbooks and coverage matrix
- QA-01.21 — DLQ reprocessing E2E (replay → success)
...
ML (Safety, Acuity, Scribe) — ML-01/ML-02/ML-03/ML-04
- ML-01.1 — Curate sample dataset + red-flag labeling schema
- ML-01.2 — NER model setup (BioClinicalBERT) + inference wrapper
- ML-01.3 — Emergency classifier head + threshold tuning (config-driven)
- ML-01.4 — Safety decision function (combine NER/cls + thresholds)
...
- ML-01.14 — Concurrency & backpressure (worker limits, time budgets, 429/503)
...
- ML-01.20 — Bias/fairness audit and mitigations
- ML-01.21 — Threshold calibration + temperature scaling (reproducible tuning)
- ML-01.22 — Adversarial input and toxicity guard
...
Telephony/Voice (TV-01/TV-02)
- TV-01.1 — Telephony ingest adapter skeleton (capture audio/metadata)
- TV-01.2 — ASR client integration; map output to CallTranscribed schema
- TV-01.3 — Publish telephony.call.transcribed envelope (EventEnvelope)
- TV-01.4 — Emergency transfer stub with config flags (enabled/disabled)
- TV-01.5 — Contract-first: telephony schemas + validators
- TV-01.6 — ASR client guardrails (timeout/retry/jitter/CB; SSRF allowlist; TLS)
- TV-01.7 — Streaming ingest → Object Store (Binary/DocumentReference)
- TV-01.8 — Language detection + diarization options
- TV-01.9 — Intent classification integration + publish intent.classified
- TV-01.10 — Idempotency & dedupe (per-call)
- TV-01.11 — Backpressure & rate limits
- TV-01.12 — Observability (correlationId, metrics, spans; PHI-safe logs)
- TV-01.13 — Health/readiness + graceful shutdown
- TV-01.14 — Fault injection tests (timeouts, CB-open, network blips)
- TV-01.15 — Privacy & retention policy (audio and events)
- TV-01.16 — Sandbox playback harness (offline CI)
- TV-02.1 — Intent classifier client skeleton + env (endpoint/keys)
- TV-02.2 — Map intents → triage input and publish telephony.intent.classified
- TV-02.3 — Callback window offering logic using config (by priority)
- TV-02.4 — IVR prompts and language options (i18n, config-driven)
