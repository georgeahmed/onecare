...
export async function callWithGuard<T>(
  ...
  if (breaker.state === 'open') {
    if (startedAt - breaker.openedAt >= cbCooldownMs) {
      ...
      log('booking.guard.cb.half_open', name, correlationId);
    } else {
      log('booking.guard.cb.reject', name, correlationId, { sinceMs: startedAt - breaker.openedAt });
  ...
  for (;;) {
    ...
    try {
      ...
      if (breaker.state !== 'closed') {
        ...
        breaker.failures = 0;
        log('booking.guard.cb.closed', name, correlationId);
      }
      return result;
    } catch (error) {
      clearTimeout(timer);
      const elapsedAttempt = now() - startedAttempt;
      const timeout = controller.signal.aborted;
      if (timeout) {
        log('booking.guard.timeout', name, correlationId, { attempt, elapsedMs: elapsedAttempt });
      }

      const retryable = timeout || isRetryable(error);
      if (retryable && attempt < maxAttempts) {
        const backoff = baseDelayMs * Math.pow(2, attempt - 1) + jitter(baseDelayMs, random);
        log('booking.guard.retry', name, correlationId, {
          attempt,
          backoffMs: backoff,
      ...
      if (breaker.failures >= cbFailureThreshold) {
        ...
        log('booking.guard.cb.open', name, correlationId, { failures: breaker.failures });
      }
      throw error;
    }
  }
...
export function resetBookingGuardBreakers(): void {
  breakers.clear();
}
