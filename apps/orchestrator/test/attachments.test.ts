...
function installAllowAllSecurity(): void {
  ...
  const consent = vi.fn<SecurityServices['checkConsent']>().mockImplementation(
    async (patientId, purpose, resources) => {
      ...
      setConsentEvidenceForTest(patientId, purpose, evidence);
