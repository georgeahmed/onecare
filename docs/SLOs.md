...
| Analytics | Materialiser p95 ≤ **120 s** | `histogram_quantile(0.95, sum(rate(analytics_ingest_lag_ms_bucket[15m])) by (metricName, le))` | `infra/monitoring/alerts/slo-analytics.yaml` (`AnalyticsMaterialiserLatency`) | `docs/runbooks/oncall.md#analytics-materialiser` |
| Telephony | Call ingest p95 ≤ **10 s** | `histogram_quantile(0.95, sum(rate(telephony_http_duration_ms_bucket{path="/calls"}[10m])) by (le))` | `infra/monitoring/alerts/slo-telephony.yaml` (`TelephonyIngressLatency`) | `docs/runbooks/oncall.md#telephony-ingress` |
