...
- [x] PII/PHI redaction for logs, metrics, and audit; secrets never logged. (BE‑01.14, BE‑01.20)
- [x] SSRF allowlist enforced for outbound hosts. (BE‑01.12)
- [x] Rate limiting (pre/post auth) returns 429 envelopes; safe defaults via config. (BE‑01.12, BE‑01.17)
- [x] Input sanitation: strict content-type, size limits, header normalization. (BE‑01.10)
...
- [x] Reliable publish with ack/timeout/retry; DLQ event on max attempts. (BE‑01.3H, BE‑01.16)
...
- [ ] SLOs defined (availability, p50/p95 latency) with baselines documented; initial alerts configured. (BE‑01.21)
  - ICS Hub routing/ack latency SLOs recorded in `docs/ics-hub-SLO.md`; metrics validated via `npx vitest run apps/ics-hub/test`.
  - Orchestrator `/safety-check` throughput and latency baselines captured in `docs/perf/orchestrator-baselines.md`.

Performance & Backpressure
- [x] Backpressure controls: global/per‑state concurrency caps; 429/503 on saturation with retry‑after. (BE‑01.14)
- [x] Load test shows stable p50/p95 under target concurrency; mapping hot paths micro‑benchmarked. (BE‑01.21)

Operations
- [x] Health/liveness and readiness endpoints; readiness reflects dependencies and backpressure. (BE‑01.18)
...
- [x] Fault injection tests: timeouts, partial failures, saturation/backpressure. (BE‑01.19)
- [ ] Contract tests for critical envelopes/payloads succeed; no network in unit tests. (BE‑01.15)

Documentation
- [x] ADRs for orchestrator design, error taxonomy, idempotency, guardrails. (BE‑01.22)
- [x] Orchestrator README updated with endpoints, env vars, examples, error envelopes. (BE‑01.22)
- [x] USAGE updated for commands, scripts, run modes. (BE‑01.22)

Go/No‑Go Criteria
- [ ] Typecheck/lint/tests green across monorepo; coverage threshold met for orchestrator.
- [ ] Container image builds; SBOM artifacts uploaded; vulnerability/license scans pass policy gate; images signed.
