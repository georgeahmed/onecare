...
- Enforce message size policy at the adapter: payloads above the JetStream limit trigger compression (gzip JSON) before publish; if still too large we reject with `message_size_limit_exceeded` and record `bus.msg.too_large` for SRE visibility.
